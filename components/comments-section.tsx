    ...
    <div className="mt-8 space-y-8 border-t border-border/50 pt-8">
      ...
      {!replyTo && (
        <form
          ...
          className="space-y-4 rounded-xl border border-border/50 bg-card/50 p-6 shadow-sm"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      {isReplyFormActive && (
        <form
          onSubmit={handleSubmit}
          className="ml-14 animate-in zoom-in-95 space-y-3 rounded-xl border border-primary/20 bg-primary/5 p-4 duration-200"
        >
          <div className="flex items-center justify-between">
          ...
          {error && <div className="rounded-lg bg-destructive/10 p-2 text-[10px] text-destructive">{error}</div>}
        </form>
      )}
        ...
        <div className="ml-14 space-y-4 border-l-2 border-border/20 pl-4">
          {replies.map((reply) => (
            <CommentItem
              ...
              onReply={onReply}
              isReplyFormActive={activeReplyId === reply.id}
              formatDate={formatDate}
              handleSubmit={handleSubmit}
              isSubmitting={isSubmitting}
              success={success}
